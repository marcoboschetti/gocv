...
//GetPerspectiveTransform - Custom addition
func GetPerspectiveTransform(src, dst Mat) Mat {
	return Mat{p: C.GetPerspectiveTransform(src.p, dst.p)}
}

//WarpPerspective - Custom addition
func WarpPerspective(src, dst, rotMat Mat, sz image.Point) {
	pSize := C.struct_Size{
		width:  C.int(sz.X),
		height: C.int(sz.Y),
	}
	C.WarpPerspective(src.p, dst.p, rotMat.p, pSize)
	return
}

//CopyMakeBorder - Custom addition
func CopyMakeBorder(src, dst Mat, top, bottom, left, right int, borderType BorderType) {
	C.CopyMakeBorder(src.p, dst.p, C.int(top), C.int(bottom), C.int(left), C.int(right), C.int(borderType))
	return
}

//CornerHarris
func CornerHarris(src, dst Mat, blockSize, ksize int, k float64) {
	C.CornerHarris(src.p, dst.p, C.int(blockSize), C.int(ksize), C.double(k))
	return
}

func Subtract(src1, src2, dst Mat) {
	C.Subtract(src1.p, src2.p, dst.p)
	return
}

func ConvertScaleAbs(src, dst Mat) {
	C.ConvertScaleAbs(src.p, dst.p)
	return
}

func Sobel(src, dst Mat, ddepth, dx, dy int) {
	C.Sobel(src.p, dst.p, C.int(ddepth), C.int(dx), C.int(dy))
	return
}

func MinAreaRectAngle(contour []image.Point) float64 {
	cPointArray := make([]C.struct_Point, len(contour))
	for i, r := range contour {
		cPoint := C.struct_Point{
			x: C.int(r.X),
	...
	result := C.MinAreaRectAngle(cContour)
	return float64(result)
}
