...
void WarpPerspective(Mat src, Mat dst,  Mat rot_mat, Size dsize){
   cv::Size sz(dsize.width, dsize.height);
   cv::warpPerspective(*src, *dst, *rot_mat, sz);  
}
...
// This function is custom made for Vortex
double MinAreaRectAngle(Contour con) {
    std::vector<cv::Point> pts;
    for (size_t i = 0; i < con.length; i++) {
        pts.push_back(cv::Point(con.points[i].x, con.points[i].y));
    }
    cv::RotatedRect calculatedRect = cv::minAreaRect(pts);

    double angle = calculatedRect.angle;
